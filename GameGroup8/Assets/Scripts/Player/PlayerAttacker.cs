    ...
    void Update()
    {
        ...
        if (!Base)
        {
            ...
            if (!currentWeapon.getIfElectric() && currentWeapon.getIfAutomatic() && Input.GetMouseButton(0) && Time.time > nextAttack && !currentWeapon.getIfMelee())
            {
                ...
                // analytisc test Unity vs GameAnalytics
                Analytics.CustomEvent("shoot", new Dictionary<string, object>
                {
                    { "type bullet", currentWeapon.getType().toString() },
                    { "weap damage", currentWeapon.getWeaponDamage() }                    
                });

                GameAnalytics.NewErrorEvent(GA_Error.GAErrorSeverity.GAErrorSeverityInfo, "shoot");
