...
public class MiniMapScript : MonoBehaviour {
	...
	void Update(){
		float xPlayer = 1.5f * (PlayerController.getPosition ().x);
		float yPlayer = 1.5f * (PlayerController.getPosition ().z);
		...
		if (enemies.Count > 0) {
			for(int i = 0; i < enemies.Count; i++){
				...
				float xEnemy = 1.5f * (enemies[i].gameObject.transform.position.x);
				float yEnemy = 1.5f * (enemies[i].gameObject.transform.position.z);
